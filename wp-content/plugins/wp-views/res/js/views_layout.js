...
function wpv_update_layout_rows() {
    ...
    jQuery('#view_layout_fields_table tbody tr').each(function(){
      ...
      //  jQuery(this).children('td:nth-child(5)').children('input').attr('id', 'wpv_field_edittext_'+row).attr('name', '_wpv_layout_settings[fields][edittext_'+row+']'); EDITTEXT
...
function on_add_field_wpv(menu, name, text) {
    ...
    // td += '<td width="66px"><a href="#edit" onclick="wpv_show_edit_shortcode_input();">Edit</td>'; EDITTEXT
...
function on_generate_wpv_layout(force) {
    ...
    while (jQuery('#wpv_field_row_' + temp_index).length != 0) {
        ...
        if (field_type.indexOf(wpv_field_text) == 0) {
            ...
            header_name = 'post-field-' + field_type.substring('Types - '.length);
        ...
        // taxonomy search, some prefix preset
        	...
        	// handle taxonomies
        	field_type = '[wpv-taxonomy type="' + field_type + '" separator=", " format="link" show="name"]';
        ...
        if ((query_type == 'posts' && (add_type == 'custom' || add_type == 'posts')) || (add_type == 'taxonomy')) {
