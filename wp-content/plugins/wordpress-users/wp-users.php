...
function display_user() {  
	...
	if ( $curauth ) {
		...
		if ($curauth->wp_s2member_custom_fields["occupation"]) {
             $html .= "<p><strong>Occupation:</strong> ".$curauth->wp_s2member_custom_fields["occupation"]." \n";
       }	

		if ($curauth->wp_s2member_custom_fields["projects"]) {
             $html .= "<p><strong>Projects:</strong> ".$curauth->wp_s2member_custom_fields["projects"]." \n";
       }
       
       	if ($curauth->wp_s2member_custom_fields["twitter"]) {
             $html .= "<p><strong>Twitter</strong> ".$curauth->wp_s2member_custom_fields["twitter"]." \n";
       }
       
       	if ($curauth->wp_s2member_custom_fields["facebook"]) {
             $html .= "<p><strong>Facebook</strong> ".$curauth->wp_s2member_custom_fields["facebook"]." \n";
       }
       
       	if ($curauth->wp_s2member_custom_fields["google-plus"]) {
             $html .= "<p><strong>Google+</strong> ".$curauth->wp_s2member_custom_fields["google-plus"]." \n";
       }
	...
	<div class="wrap">
		...
		<form name="wpu_admin_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
            ...
            <table>
				...
				<tr>
					<td colspan="3"><input name="wpu_roles_s2member_level1" type="checkbox" value="yes" <?php checked('yes', get_option('wpu_roles_s2member_level1')); ?> />&nbsp; <?php _e("Participants" ); ?></td>
				</tr>                
                <tr>
                	<td>&nbsp;</td>
                	<td>&nbsp;</td>
